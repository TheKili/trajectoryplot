  ...
  factory: function(el, width ,height) {
    ...
    return {
      ...
      renderValue: function(input) {
        ...
        // interface to select a data set
        
		
		for(let x of input){
				
			let [data, {alphabet,innerPadding, ids, groupColorList, groupKey, cpak, ylab, names, margins, padding, position, layout, strokeStyle, dataArea, alpha}] = x
			
			
			let child = document.createElement("div")
			child.style.height =  typeof height == "string" ? height : `${height}px`
			child.style.width = typeof width == "string" ? width : `${width}px`
			el.append(child)
			console.log(data)
			new TrajectoryPlot({
				target: child,
				props: {
				  ...
				  groupColorList: groupColorList ?? undefined,
